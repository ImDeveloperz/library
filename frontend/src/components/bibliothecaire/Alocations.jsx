...
import {columns, statusOptions} from "../data/dataLocations.js";
import {
    Button,
    Chip,
    Dropdown,
...
const INITIAL_VISIBLE_COLUMNS = ["id","nomUser","client","titreDocument", "dateDebut","dateFin", "statueAlocation", "actions"];

const Alocations = (props) => {
    const {auth} = useAuth()
    ...
    const filteredItems = React.useMemo(() => {
        ...
        if (statusFilter !== "all" && Array.from(statusFilter).length !== statusOptions.length) {
            filteredUsers = filteredUsers.filter((user) =>
                Array.from(statusFilter).includes(user.statueAlocation),
            );
        }
    ...
    const retourner = async (user) => {
        console.log(user)
        try {
            const response = await axios.delete('/locations?id='+user.id,{
                headers:{
                    "Authorization": "Bearer " + auth.token,
                }
            })
            //envoer notification aux client
            const res = await axios.post('/notifications/sendFromLocation',{
                userId:user.userId,
                message:"votre location est terminer",
    ...
    const notify = async (user) => {
        ...
        try {
            ...
            const res = await axios.post('/notifications/sendFromLocation',{
                userId:user.userId,
                message:"votre location est " + user.statueAlocation,
                dateRetour : user.dateFin,
                dateDepart : user.dateDebut,
                bibliothecaireEmail:auth.email,
                documentId : user.documentId
    ...
    const renderCell = React.useCallback((user, columnKey) => {
        ...
        switch (columnKey) {
            ...
            case "statueAlocation":
                return (
                    <Chip
                        className="caReturnpitalize border-none gap-1 text-default-600"
                        color={statusColorMap[user.statueAlocation]}
                        size="sm"
                        variant="dot"
                    >
                        {cellValue}
